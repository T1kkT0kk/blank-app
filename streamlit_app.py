...
import json
import threading # NEW: For instant background processing [cite: 2026-02-28]
...
# 3. Permanent Memory vs Session State
...
# BACKGROUND SYNC WORKERS: These prevent the UI from "freezing" [cite: 2026-02-28]
def bg_append(row_data):
    ws_logs.append_row(row_data)

def bg_delete(row_idx):
    ws_logs.delete_rows(row_idx)

def bg_update(row_idx, row_data):
    ws_logs.update([row_data], f"A{row_idx}")
...
# ASYNC SYNC: Updates memory first, then kicks off a background thread [cite: 2026-02-28]
    ...
    if is_weekend:
        ...
        with st.container(border=True):
            ...
            for idx, entry in day_entries.iterrows():
                ...
                # INSTANT DELETE: UI update + background cloud call [cite: 2026-02-28]
            ...
            # INSTANT ADD: UI update + background cloud call [cite: 2026-02-28]
